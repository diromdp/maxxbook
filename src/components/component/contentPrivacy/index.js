...
const ContentPrivacy = () => {
    ...
    const getData = async () => {
        await axios.get(`${urlAPI}backend/settings?keys=page.description_privacy,page.description_privacy_id`, {
            headers: {
    ...
    const selectedEditor = detailSEO && detailSEO.filter(x => x.key === 'page.description_privacy')
    const selectedEditorID = detailSEO && detailSEO.filter(x => x.key === 'page.description_privacy_id')
