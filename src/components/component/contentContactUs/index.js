...
const ContentContactUs = () => {
    ...
    const getData = async () => {
        await axios.get(`${urlAPI}backend/settings?keys=page.description_contact,page.description_contact_id`, {
            headers: {
    ...
    const selectedEditor = detailSEO && detailSEO.filter(x => x.key === 'page.description_contact')
    const selectedEditorID = detailSEO && detailSEO.filter(x => x.key === 'page.description_contact_id')
        ...
        <>
            {
                isLoading ?
                    <div className="flex flex-col space-y-3 mb-3">
                        <Skeleton className="h-5 w-fullrounded-xl" />
