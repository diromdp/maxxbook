...
const Category = () => {
    ...
    const updatePagination = async (val) => {
        ...
        if (url_string) {
            ...
            await axios.get(`${urlAPI}backend/admin/settings?q=${filterUser.q}&page=${page}&perPage=${filterUser.perPage}&sortBy=${filterUser.sortBy}&sortDirection=${filterUser.sortDirection}`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': "application/json",
                    "Authorization": `Bearer ${cookies.token}`
    ...
    const searchItems = async (value, _e, info) => {
        ...
        await axios.get(`${urlAPI}backend/admin/settings?q=${value}&page=${1}&perPage=${filterUser.perPage}&sortBy=${filterUser.sortBy}&sortDirection=${filterUser.sortDirection}`, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': "application/json",
                "Authorization": `Bearer ${cookies.token}`
