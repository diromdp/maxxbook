...
const Category = () => {
    ...
    const getData = async (val) => {
        await axios.get(`${urlAPI}backend/admin/categories/paginated?q=${filterUser.q}&cursorEnabled=${filterUser.cursorEnabled}&cursor=${filterUser.cursor}&page=${filterUser.page}&perPage=${filterUser.perPage}&sortBy=${filterUser.sortBy}&sortDirection=${filterUser.sortDirection}`, {
            headers: {
    ...
    const updatePagination = async (val) => {
        ...
        if (url_string) {
            ...
            await axios.get(`${urlAPI}backend/admin/categories/paginated?q=${filterUser.q}&page=${page}&perPage=${filterUser.perPage}&sortBy=${filterUser.sortBy}&sortDirection=${filterUser.sortDirection}`, {
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': "application/json",
                    "Authorization": `Bearer ${cookies.token}`
    ...
    const searchItems = async (value, _e) => {
        ...
        await axios.get(`${urlAPI}backend/admin/categories/paginated?q=${value}&page=${1}&perPage=${filterUser.perPage}&sortBy=${filterUser.sortBy}&sortDirection=${filterUser.sortDirection}`, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': "application/json",
                "Authorization": `Bearer ${cookies.token}`
    ...
    const editData = (data) => {
        router.push(`/admin/category/${data.id}?id=${data.id}`);
    }

    const deleteData = async () => {
        await axios.delete(`${urlAPI}backend/admin/categories`, {
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${cookies.token}`
            },
