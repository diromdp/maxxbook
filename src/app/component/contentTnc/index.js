...
const ContentTnc = () => {
    ...
    const getData = async () => {
        await axios.get(`${urlAPI}backend/settings?keys=page.description_term,page.description_term_id`, {
            headers: {
    ...
    const selectedEditor = detailSEO && detailSEO.filter(x => x.key === 'page.description_term')
    const selectedEditorID = detailSEO && detailSEO.filter(x => x.key === 'page.description_term_id')
